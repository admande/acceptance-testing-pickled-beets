...
feature "User checks a recipe's deliciousness", %(
  ...
  scenario "user submits a recipe name containing 'pickled beets'" do
    ...
    fill_in('recipe_name', :with => 'Magic pickled beets')
    click_button 'Submit'
    expect(page).to have_content('"Magic pickled beets" is a delicious recipe!')
  end

  scenario "user submits a recipe name without 'pickled beets'" do
    visit '/'
    fill_in('recipe_name', :with => 'stupid brussel sprouts')
    click_button 'Submit'
    expect(page).to have_content('"stupid brussel sprouts" is not a delicious recipe!')
  end
  ...
  scenario "user submits an empty form" do
    visit '/'
    click_button 'Submit'
    expect(page).to have_content("Error! You can't submit an empty recipe name!")
  end
